...
# Transações por tempo        
    with st.container():
        ...
        fig.add_trace(
            go.Histogram(x=edges_class_0, name="Transações normais",
                        marker_color='#0C3559', hoverinfo='text',
                        hovertext=f'{edges_class_0}', nbinsx=48),
            row=1, col=1
        )

        # Adicionar histograma para a classe 1 com cor personalizada (azul) e hovertext com a quantidade de transações
        fig.add_trace(
            go.Histogram(x=edges_class_1, name="Transações fraudulentas",
                        marker_color='#3698BF', hoverinfo='text',
                        hovertext=f'{edges_class_1}', nbinsx=48),
            row=2, col=1
        )
